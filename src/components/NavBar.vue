<template>
	<div>
		<v-app-bar clipped-left flat app :dense="size.xs" :hide-on-scroll="size.smAndDown" class="navbar">
			...
			<v-scroll-x-transition hide-on-leave>
				<div v-show="!titleTransitioning" :class="['text--secondary', 'text-truncate', 'text-capitalize', size.xs ? 'subtitle-1' : 'display-1']">{{ currentPage }}</div>
			</v-scroll-x-transition>
			<v-spacer></v-spacer>
			<v-toolbar-items class="hidden-sm-and-down">
				<v-btn :v-show="size.smAndUp" v-for="route in routes" :key="route.name" text :to="route.link">
			...
			<v-toolbar-items>
				...
				<v-btn icon v-if="userLogged" to="/user">
					<img style="width:45px;height:45px;border-radius:50%" :src="user.photoURL" :alt="user.displayName" />
				</v-btn>
			</v-toolbar-items>
		</v-app-bar>

		<v-bottom-navigation class="hidden-md-and-up" fixed grow>
			<v-btn small :v-show="size.smAndUp" v-for="route in routes" :key="route.name" text :to="route.link">
				<span class="hidden-xs-and-down">{{ route.name }}</span>
