<template>
	<div>
		<v-form @submit.prevent="onSubmit" ref="form" lazy-validation>
			...
			<v-textarea outlined rows="15" class="text-area text-no-wrap pt-3 mt-n3" v-model="tempSource.text" label="Song text with chords" :rules="rules" auto-grow></v-textarea>
...
export default {
	...
	methods: {
		...
		splitLinesAndFindChords(text, chordsAboveText) {
			...
			lines.forEach((line) => {
				let lineType = "lyrics";
				let chords = [];
				if (/\s*(Chorus|Verse\s+\d|Bridge|Ending)\s*/i.test(line)) {
					lineType = "delimiter";
					...
					while ((bracketMatch = /\[(.*?)\]/.exec(line))) {
						...
						while ((match = this.chordsInBracketRegex.exec(bracketMatch[1]))) {
							if (chordsInBracket.length != 0) {
								...
								chordsInBracket.push([lastChord[0] + lastChord[1].length + 1, match[0]]);
							} else {
	...
	computed: {
		formatedSong() {
			...
			// if (this.tempSource.trimLines) finalLineArray = this.trimLines(finalLineArray);
