<template>
  <fullscreen
    ...
    :background="$vuetify.theme.dark ? '#121212' : '#ffffff'"
  >
    ...
    <v-sheet style="position: relative" :class="type === 'modal' ? 'elevation-0' : 'elevation-2'">
      <v-toolbar height="50px" class="elevation-0">
        <v-toolbar-items>
          ...
          <v-btn text :icon="collapse" class="mx-n2">
            <!-- <v-icon left>mdi-format-font-size-increase</v-icon> -->
            <span v-if="!collapse">Transpose</span>
            <span v-else>0</span>
          </v-btn>
          <v-btn icon>
            ...
            <v-list class="py-0">
              ...
              <v-list-item>
                ...
                <v-list-item-content>
                  <v-select
                    @click.stop.prevent
                    v-model="currentFontSizePreference"
                    :items="fontSizePreferences"
                    label="Font size"
                    class="my-n2 py-0"
      ...
      <div class="px-7 py-3">
        ...
        <div
          v-if="songValid && song.sections.length > 0"
          :class="['text--primary', 'song-sheet', 'mt-3', multipleColumns ? 'multiple-columns' : '']"
          ref="songSheet"
        >
          <v-sheet
            :class="['mb-3 ', 'section', multipleColumns ? 'multiple-columns mt-3' : '']"
            :style="{ 'font-size': sectionFontSize + 'px' }"
            @keydown="dynamicSectionFontSize = maxFontSize"
            v-for="(section, i) in songSections"
            :key="i"
          >
...
export default {
      ...
      (section.lines || []).forEach((e) => {
        ...
        if (e.chords?.length) {
          e.chords.forEach((ch, i) => {
            if (i == 0) {
              chordLine = chordLine.insert(
                ch[0],
                "<span class='chord'>" + ch[1] + "</span>"
              );
            } else {
              chordLine = chordLine.insert(
                chordLine.length +
                  (ch[0] - (e.chords[i - 1][0] + e.chords[i - 1][1].length)),
                "<span class='chord'>" + ch[1] + "</span>"
              );
            }
          });
  ...
  computed: {
    ...
    minFontSize() {
      switch (
        this.fontSizePreferences.indexOf(this.currentFontSizePreference)
      ) {
        case 0:
          return 9;
        case 1:
    ...
    sectionFontSize() {
      if (!this.multipleColumns) {
        return Math.min(
          Math.max(this.minFontSize, 16),
          this.dynamicSectionFontSize
        );
      } else {
    ...
    songValid() {
      return (
        ...
        this.song.author !== undefined &&
        this.song.sections !== undefined
      );
    },
    ...mapGetters({
