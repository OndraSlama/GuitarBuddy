<template>
  <div class="mt-xs-4 pl-xs-0 pl-md-6">
    <v-row align-content="center">
      ...
      <v-col class="text-start" align-self="center">
        <v-btn-toggle v-model="multipleColumns" rounded mandatory>
          ...
          <v-btn @click="sectionFontSize = maxFontSize;">
            <span>Dynamic</span>
          </v-btn>
        </v-btn-toggle>
      </v-col>
  ...
  methods: {
    ...
    updateFontSize() {
      if (this.multipleColumns) {
        ...
        if (overflowDiff < 0 && this.sectionFontSize > 9) {
          this.sectionFontSize *= 0.95;
        }
      } else {
        this.sectionFontSize = 16;
      }
    },

    onResize() {
      this.sectionFontSize = this.maxFontSize;
      this.updateFontSize();
    },
  },
