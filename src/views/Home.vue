    ...
    <v-container>
      <v-row justify="center">
        <v-col
          cols="12"
          sm="6"
          :style="size.smAndUp ? { minWidth: 'clamp(0px, 800px, 100%)' } : {}"
        >
          <p
            :class="[
              'text--secondary text-center mb-5 mb-sm-10',
              size.smAndUp ? 'display-3' : 'display-1',
            ]"
            align-self
          >
            Browse Public Songs
          ...
          >
            <v-toolbar-items>
              <v-btn
                text
                :icon="size.xs"
                :color="filters.groupByAuthor ? '' : 'primary'"
                @click="filters.groupByAuthor = false"
              >
              ...
              <v-btn
                ...
                :icon="size.xs"
                :color="filters.groupByAuthor ? 'primary' : ''"
                @click="filters.groupByAuthor = true"
              >
            ...
            <v-toolbar-items>
              <v-menu rounded="large" transition="slide-y-transition" bottom>
                ...
                <v-list class="py-0">
                  ...
                  <v-list-item @click.stop="onAuthorsAlph">
                    ...
                    <v-list-item-icon>
                      <v-icon
                        ...
                        :color="
                          filters.orderOption == 1 || filters.groupByAuthor
                            ? 'primary'
                            : ''
                        "
                        >mdi-sort-alphabetical-ascending</v-icon
                  ...
                  <v-list-item @click.stop="onSongsForks">
                    <v-list-item-title>Popularity</v-list-item-title>
                    <v-list-item-icon>
                      <v-icon
                        v-show="filters.songsForksOrder"
                        :color="filters.orderOption == 3 ? 'primary' : ''"
          ...
          <v-sheet class="elevation-1">
            ...
            <v-list v-if="!publicSongListLoading">
              <v-scroll-x-transition group hide-on-leave>
                ...
                ></v-list-item-group>
                <component
                  :is="filters.groupByAuthor ? 'v-list-group' : 'v-list'"
                  v-for="group in groupedSongs(filters)"
                  :key="group.group"
                  :prepend-icon="
                    filters.groupByAuthor ? 'mdi-account-circle-outline' : ''
                  "
                >
                  ...
                  <v-scroll-x-transition group hide-on-leave>
                    ...
                    <v-list-item
                      ...
                      :two-line="!filters.groupByAuthor"
                      @click.stop="openSongDialog(song)"
                      :key="song.id"
                    >
                      <v-list-item-content>
                        <v-list-item-title>{{ song.title }}</v-list-item-title>
                        <v-list-item-subtitle v-if="!filters.groupByAuthor">{{
                          song.author
                      ...
                      <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                          <div
                            v-show="song.forks && size.smAndUp"
                            class="mr-5"
                            style="display: absolute; right: 40%"
                            v-bind="attrs"
                            v-on="on"
                          >
                      ...
                      <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                          <div
                            v-show="song.modifiedAt && size.smAndUp"
                            class="mr-5"
                            v-bind="attrs"
                            v-on="on"
                          >
                        ...
                        <v-tooltip top>
                          <template v-slot:activator="{ on, attrs }">
                            <v-fab-transition hide-on-leave>
                              <v-btn
                                v-if="!alreadyInCollection(song.id)"
                                :disabled="
                                  !userLogged || alreadyInCollection(song.id)
                                "
                                fab
                                x-small
                                class="elevation-0 primary mt-1"
                                v-bind="attrs"
                                v-on="on"
                                @click.stop.prevent="addToCollection(song)"
                              >
...
export default {
  ...
  methods: {
    ...
    onSongsForks() {
      this.filters.songsForksOrder = !this.filters.songsForksOrder;
      this.filters.orderOption = 3;
    },
