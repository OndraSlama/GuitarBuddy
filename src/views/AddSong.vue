<template>
	<div>
		<div>
			<p class="text-h3 text--primary font-weight-light text-sm-h2 mb-6">Add new song</p>
			<v-row>
				<v-col cols="12" md="6" lg="4">
					<v-form>
						<v-text-field class="my-3" v-model="songInput.title" label="Song Title" :rules="rules" hide-details="auto" outlined></v-text-field>
						<v-text-field v-model="songInput.author" label="Author" outlined></v-text-field>
						<v-textarea solo class="monospace" v-model="songInput.text" label="Song text with chords" :rules="rules" auto-grow></v-textarea>
						...
						<v-row align-content="center">
							...
							<v-col class="text-center">
								<v-btn @click="importSong" rounded large color="primary" :disabled="!validInput">Import</v-btn>
							</v-col>
						</v-row>
					</v-form>
				</v-col>
				...
				<v-col cols="12" md="6" lg="8">
					<SongSheet :song="parseSong()" class="pl-md-6"></SongSheet>
				</v-col>
			</v-row>
		</div>
	</div>
...
export default {
	...
	methods: {
		importSong() {
			...
			console.log(this.$store.getters.getSongs);
		},

		parseSong() {
			let lineArray = this.splitLinesAndFindChords(this.songInput.text, this.songInput.chordsAboveText);
			let curentSection = 0;
			let currentName = "";
			let finalLineArray = [];
	...
	computed: {
		...
		validInput() {
			if (this.songInput.title.length == 0) return false;
			if (this.songInput.text.length == 0) return false;
			return true;
		},
	},
	components: {
